 ...
 * ==================================
 */
const P = { shots: 'sh', assets: 'as', tasks: 'tk', projectmembers: 'pm', users: 'us', pages: 'pg', fields: 'fi' };
const META = 'Fields';
...
function runAlphabetFillFromMenu() {
  ...
  if (!range) {
    ui.alert('エラー', '連番を生成したい範囲を選択してから実行してください。', ui.ButtonSet.OK);
    return;
  }
  if (sh.getName().toLowerCase() !== 'shots') {
    ui.alert('エラー', 'この機能は「Shots」シートでのみ使用できます。', ui.ButtonSet.OK);
    return;
  }
  
  const shotColumn = getColumnIndexByFieldName(sh, SHOT_FIELD_NAME);
  if (shotColumn === -1 || range.getColumn() !== shotColumn) {
    ui.alert('エラー', `この機能は「${SHOT_FIELD_NAME}」列でのみ使用できます。\n（現在の対象列は ${sh.getRange(2, shotColumn).getValue()} 列です）`, ui.ButtonSet.OK);
    return;
  }
  
  if (range.getRow() < 4) {
      ui.alert('エラー', '4行目以降のセルを選択して実行してください。', ui.ButtonSet.OK);
      return;
  }
  ...
  if (!val1 || !val2 || val2 !== nextA(val1)) {
    ui.alert('エラー', '選択範囲の上2つのセルが「A」「B」のような連番になっていません。', ui.ButtonSet.OK);
    return;
  }
...
function handleHeaderEdit(e) {
  ...
  if (!fid) {
    ...
    if (lastRow > 1) {
      const fids = fieldsSheet.getRange(2, 1, lastRow - 1, 1).getValues().flat();
...
function handleDataEdit(e) {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      if (!idCell.getValue()) {
        const rowData = sh.getRange(r, 2, 1, sh.getLastColumn() - 1).getValues()[0];
        if (rowData.some(v => v !== '')) {
...
const free = a => { a.sort((x, y) => x - y); let i = 1; for (const v of a) { if (v === i) i++; else if (v > i) break; } return i; };
function genId(sh, k) {
  const lastRow = sh.getLastRow();
  if (lastRow < 3) return `${P[k]}_${pad(1, 4)}`;
  const range = sh.getRange(3, 1, lastRow - 2, 1);
  if (range.isBlank()) return `${P[k]}_${pad(1,4)}`;
  const nums = range.getValues().flat().filter(String).map(x => +x.split('_')[1]);
  const n = free(nums);
...
function getMetaRows() {
  ...
  if (lastRow < 2) return [];
  return fSheet.getRange(2, 1, lastRow - 1, 3).getValues().filter(r => r[0] && r[1] && r[2]);
}
const newFid = () => `${P.fields}_${pad(free(getMetaRows().map(r => +String(r[0]).split('_')[1] || 0)), 4)}`;
function entSheet(ent) {
  const s1 = ent.charAt(0).toUpperCase() + ent.slice(1);
  const s2 = s1.endsWith('s') ? s1 : s1 + 's';
  return SpreadsheetApp.getActive().getSheetByName(s1) || SpreadsheetApp.getActive().getSheetByName(s2);
}
...
function syncHeaders() { getMetaRows().forEach(([f, e, n]) => applyField(f, n, e)); }


// ---【維持】Shot Code関連 (Field ID基準) ---
const mkCode = v => {
  const n = [v[4], v[3], v[2], v[1]].map(x => String(x || '').replace(/\D/g, '')).filter(Boolean);
  if (!n.length) return '';
...
function recalcCodes(sh) {
    ...
    const componentFids = {
        Shot: nameToFid['shot'], Scene: nameToFid['scene'],
        Sequence: nameToFid['sequence'], Act: nameToFid['act'],
        Episode: nameToFid['episode'], ShotCode: nameToFid['shotcode']
    };
    if (!componentFids.ShotCode) return;
    const fidHeader = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0];
    const colIndex = {
        Shot: fidHeader.indexOf(componentFids.Shot), Scene: fidHeader.indexOf(componentFids.Scene),
        Sequence: fidHeader.indexOf(componentFids.Sequence), Act: fidHeader.indexOf(componentFids.Act),
        Episode: fidHeader.indexOf(componentFids.Episode), ShotCode: fidHeader.indexOf(componentFids.ShotCode)
    };
    if (colIndex.ShotCode === -1) return;
    const allData = sh.getRange(3, 1, dataRowCount, sh.getLastColumn()).getValues();
...
// --- アルファベットフィル関連 ---
const nextA = p => !p ? 'A' : p === 'Z' ? 'Za' : /Zz+$/.test(p) ? p + 'a' : p.length === 1 ? String.fromCharCode(p.charCodeAt(0) + 1) : p.slice(0, -1) + String.fromCharCode(p.slice(-1).charCodeAt(0) + 1);
function getColumnIndexByFieldName(sheet, fieldName) {
  ...
  const fidHeader = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
