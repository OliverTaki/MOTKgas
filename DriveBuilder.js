...
//   • DataHub G列 Core_values から ID / Code を取得してフォルダ同期
//   • diff / audit: DataHubとDriveの差分抽出と不足フォルダの生成
// ---------------------------------------------------------------------------
...
function listEntityFiles(req){
  var meta   = req.projectMeta||getProjectMeta();
...
function serializeFiles_(folder, kind, id){
  ...
  while (it.hasNext()){
    ...
    if (kind === 'proxies'){
      if (id && n.indexOf(id) !== 0)  continue;
      if (!/_proxy\./i.test(n))       continue;
    }
    out.push({
      id:           f.getId(),
      name:         n,
